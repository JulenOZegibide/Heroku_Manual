...
function drilldownmenu(setting){
    this.sublevelarrow={src: '../img/next.png' , width:'20px', top:'0', left:'0'} //full URL to image used to indicate there's a sub level
    this.breadcrumbarrow='../img/next.png' //full URL to image separating breadcrumb links (if it's defined)
    ...
    this.backarrow='../img/back.png' //full URL to image added in front of back LI
...
drilldownmenu.prototype.buildmenu=function($){
    ...
    this.$uls.each(function(i){ //loop through each UL
        ...
        if (i==0){ //if topmost UL
            $('<li class="backcontroltitle" id="cabecera"><input type="text" id="busqueda" name="busqueda" onkeyup="buscar(this.value)" autocomplete="off" spellcheck="false"><svg id="lupa"><path d="M18 16.5l-5.14-5.18h-.35a7 7 0 1 0-1.19 1.19v.35L16.5 18l1.5-1.5zM12 7A5 5 0 1 1 2 7a5 5 0 0 1 10 0z"></path></svg><div id="resultado" style="height: 0"></div></li>').prependTo($thisul).click(function(e){e.preventDefault()})
            thisdrill.$maindiv.css({position:'fixed', height:'100%', overflow:'hidden'}) //set main menu DIV's height to top level UL's height
